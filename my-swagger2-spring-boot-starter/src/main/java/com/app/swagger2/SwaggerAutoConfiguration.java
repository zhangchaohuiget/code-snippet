...
public class SwaggerAutoConfiguration implements BeanFactoryAware {
    ...
    private List<ResponseMessage> getResponseMessages() {
        return Arrays.asList(
                ...
                new ResponseMessageBuilder().code(500).message("系统繁忙").build(),
                new ResponseMessageBuilder().code(502).message("服务超时").build(),
                new ResponseMessageBuilder().code(401).message("会话超时，请重新登录").build(),
                new ResponseMessageBuilder().code(401).message("缺少token参数").build()
        );
    }
    ...
    private List<SecurityScheme> securitySchemes(SwaggerProperties.Authorization globalAuthorization,
        ...
        List<SwaggerProperties.ApiKey> apiKeys = CollectionUtils.isEmpty(docketApiKeys) ? globalApiKeys : docketApiKeys;
